module.exports = app => {
  class CrosController extends app.Controller {
    * index(req) {
      ...
      var originReg = /^http:\/\/(.)+.localhost.com/,
          supportOrigin = '';
      if(originReg.test(req.header.origin)) {
        supportOrigin = req.header.origin;
      }
      this.ctx.set('Access-Control-Allow-Origin', supportOrigin)
      // Credentials mode
